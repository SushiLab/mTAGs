...
Find rRNA sequences in your metagenomic sequences and annotate them using a degenerate consensus references for higher sensitivity.
...
The tool is split in to two steps. The first step uses HMM models to extract potential rRNA sequences from metagenomic data. The second step aligns these sequences against a modified SILVA database and annotates sequences taxonomically.
...
Enabling the `-b` flag will produce a `.bin` file with the taxonomic annotation of each insert.
